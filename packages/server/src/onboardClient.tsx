...
let account, device, entries = new Map(), selectedId = null, viewedPage = 1, viewedPageCount = 1, syncing = false;
...
function updatePageControls() { const controls = $('#page-controls'); controls.hidden = viewedPageCount < 2; $('#page-status').textContent = `Page ${viewedPage} of ${viewedPageCount}`; $('#previous-page').disabled = viewedPage <= 1; $('#next-page').disabled = viewedPage >= viewedPageCount; }
async function loadSelectedPage(page) { const entry = entries.get(selectedId); if (!entry) return; viewedPage = Math.min(Math.max(page, 1), viewedPageCount); viewer.setAttribute('page', String(viewedPage)); const event = await loadViewer(viewer, entry.payload.note, null); viewedPageCount = event.detail?.pageCount || 1; updatePageControls(); }
async function openEntry(id) { const entry = entries.get(id); if (!entry) return; try { message('Loading decrypted note locally…'); selectedId = id; viewedPage = 1; viewedPageCount = 1; $('#inbox-listing').hidden = true; $('#detail').hidden = false; $('#note-title').textContent = entry.payload.filename; $('#note-meta').textContent = `${entry.payload.sender} · ${Math.ceil(entry.payload.note.byteLength / 1024)} KiB`; updatePageControls(); await loadSelectedPage(1); if (!entry.read) { entry.read = true; await saveEntry({ id: entry.id, record: entry.record, read: true, receivedAt: entry.receivedAt }); renderList(); } message('Note loaded.'); } catch (error) { $('#inbox-listing').hidden = false; $('#detail').hidden = true; selectedId = null; message(`Could not open this note: ${error.message}`); } }
...
$('#previous-page').onclick = async () => { if (viewedPage > 1) await loadSelectedPage(viewedPage - 1); };
$('#next-page').onclick = async () => { if (viewedPage < viewedPageCount) await loadSelectedPage(viewedPage + 1); };
