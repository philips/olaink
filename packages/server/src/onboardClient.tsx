...
logoutButton.onclick = async () => { if (!account) return; logoutButton.disabled = true; try { message('Signing out…'); await authApi('/v1/logout', { method: 'POST' }); account = null; device = null; entries.clear(); selectedId = null; activeView = 'inbox'; $('#detail').hidden = true; $('#pairing-code').hidden = true; if (location.hash) history.replaceState(null, '', location.pathname); syncPage(); message('Signed out. This browser keeps its encrypted inbox; log back in with your passkey to read it.'); } catch (error) { message(`Could not sign out: ${error.message}`); } finally { logoutButton.disabled = false; } };
