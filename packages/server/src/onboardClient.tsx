...
async function openEntry(id) { const entry = entries.get(id); if (!entry) return; try { message('Loading decrypted note locally…'); viewer.setAttribute('autoplay', AUTOPLAY_SPEED); viewer.presentation = 'write-on-playing'; await loadViewer(viewer, entry.payload.note, null); selectedId = id; $('#detail').hidden = false; $('#note-title').textContent = entry.payload.filename; $('#note-meta').textContent = `${entry.payload.sender} · ${Math.ceil(entry.payload.note.byteLength / 1024)} KiB`; if (!entry.read) { entry.read = true; await saveEntry({ id: entry.id, record: entry.record, read: true, receivedAt: entry.receivedAt }); renderList(); } message(`Note loaded. Replaying handwriting at ${AUTOPLAY_SPEED.replace('x', '×')}.`); } catch (error) { message(`Could not open this note: ${error.message}`); } }
...
async function refresh() { try { const identity = await whoami(); if (!identity?.user_id) { account = null; device = null; render(); message('Sign in with AuthGravity to set up Ola Ink.'); return; } account = (await api('/v1/account')).account; if (account.username) { device = await dbGet(DEVICE_STORE, deviceKey()); render(); if (device) { await restoreInbox(); await sync(); } else message('Create this browser’s inbox key before sharing your address.'); } else { render(); message('This account has no Ola Ink address. Create a new Ola Ink account to continue.'); } } catch (error) { message(`Account setup is unavailable: ${error.message}`); } }
...
logoutButton.onclick = async () => { if (!account) return; logoutButton.disabled = true; try { message('Signing out…'); await authApi('/v1/logout', { method: 'POST' }); account = null; device = null; entries.clear(); selectedId = null; activeView = 'inbox'; $('#detail').hidden = true; $('#pairing-code').hidden = true; if (location.hash) history.replaceState(null, '', location.pathname); render(); message('Signed out. This browser keeps its encrypted inbox; log back in with your passkey to read it.'); } catch (error) { message(`Could not sign out: ${error.message}`); } finally { logoutButton.disabled = false; } };
$('#enroll').onclick = async () => { const button = $('#enroll'); button.disabled = true; try { message('Creating a non-extractable browser receiver key…'); const next = await getDevice(); await api('/v1/devices', { method: 'POST', body: JSON.stringify({ deviceId: next.deviceId, publicKeySpki: next.publicKeySpki }) }); device = next; render(); await restoreInbox(); message(`Browser inbox enrolled for ${account.username}. You can now share this address.`); } catch (error) { message(`Could not enroll this browser: ${error.message}`); } finally { button.disabled = false; } };
...
$('#delete').onclick = async () => { if (!selectedId || !confirm('Delete this encrypted local inbox copy? This does not delete the sender’s copy or undo delivery.')) return; await removeEntry(selectedId); entries.delete(selectedId); selectedId = null; $('#detail').hidden = true; renderList(); message('Local encrypted inbox copy deleted.'); };
