...
describe('WrtnCore', () => {
  it('connects, joins echo, sends a stroke, and renders the echo reply', async () => {
    ...
    // Echo reply should have been rendered into the note (offset +384, gray).
    await settle(30);
    ...
    const pts = await echoed!.points!.getRange(0, 10);
    expect(pts).toEqual([
      { x: 1384, y: 2384 },
      { x: 1584, y: 2584 },
    ]);
    ...
    // Loop guard: the inserted echo stroke must NOT be re-sent.
